...
function CheckUserID($uid, $msgtitle='用户名', $ckhas=TRUE)
{
    ...
    if($ckhas)
    { if(strlen($uid) < 5) return $msgtitle."过短";
	  if(strlen($uid) > 8 and is_numeric($uid)) return "纯数字不能大于8位";
	  if($msgtitle=="用户id" and preg_match("/[^a-z0-9]/i",$uid)) return $msgtitle."必须为数字或英文";
	   if($msgtitle=="昵称" and is_numeric($uid)) return $msgtitle."不能为纯数字";
        $row = $dsql->GetOne("SELECT * FROM `#@__member` WHERE userid LIKE '$uid' ");
		$row2 = $dsql->GetOne("SELECT * FROM `#@__member` WHERE uname LIKE '$uid' ");
        if(is_array($row) or is_array($row2)) return $msgtitle."已存在！";
    }
    return 'ok';
}
