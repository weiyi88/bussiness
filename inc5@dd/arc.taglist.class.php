...
class TagList
{
    ...
    function CountRecord()
    {
        ...
        if($this->TotalResult==-1)
        {
            ...
            //更新浏览量和记录数
            $upquery = "UPDATE `#@__tagindex` SET total='{$row['dd']}',count=count+1,weekcc=weekcc+1,monthcc=monthcc+1 WHERE id LIKE '{$this->Tag}' ";
            ...
            {
                $this->dsql->ExecuteNoneQuery("UPDATE `#@__tagindex` SET weekcc=0,weekup='{$ntime}' WHERE id LIKE '{$this->Tag}' ");
            }

            //月统计
            if(ceil( ($ntime - $this->TagInfos['monthup'])/$oneday ) > 30)
            {
                $this->dsql->ExecuteNoneQuery("UPDATE `#@__tagindex` SET monthcc=0,monthup='{$ntime}' WHERE id LIKE '{$this->Tag}' ");
            }
        }
        ...
        for($i=0; $i<$row; $i++)
        {
            ...
            for($j=0; $j<$col; $j++)
            {
                if($row = $this->dsql->GetArray("al"))
                {
                    ...
                    //处理一些特殊字段
					$row["wgeshi"]=GetArcList($row["id"],"wgeshi");
